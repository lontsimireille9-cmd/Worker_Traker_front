...
export function getTaskCompletionDate(task) {
  ...
  const completion = [...(task.statusHistory || [])].reverse().find((entry) => entry.status === "COMPLETED");
  return completion?.changedAt || null;
}

export function getTaskDisplayDate(task) {
  return getTaskCompletionDate(task) || task?.createdAt;
}
