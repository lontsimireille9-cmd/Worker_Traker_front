...
export default function MessageList({ messages, currentUserId, loading, onLoadOlder, hasMore }) {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-canvas px-3 py-4 sm:px-5">
      {hasMore && <div className="mb-4 text-center"><Button type="Button" variant="ghost" onClick={onLoadOlder} disabled={loading} className="rounded-full border border-line bg-white px-3 py-1.5 text-xs font-medium text-primary hover:bg-surface-2">{loading ? 'Chargement...' : 'Charger les anciens messages'}</Button></div>}
