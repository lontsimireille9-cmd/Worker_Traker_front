...
export default function ChatWindow({ conversation, messages, currentUserId, loading, sending, hasMore, onLoadOlder, onSend, onBack, onOpenGroupInfo }) {
  ...
  return (
    <section className="flex h-full min-h-0 w-full min-w-0 flex-1 flex-col bg-canvas">
      <header className="flex min-h-[68px] items-center gap-3 border-b border-line bg-surface px-3 sm:px-5">
        <Button type="Button" variant="ghost" onClick={onBack} className="flex h-9 w-9 items-center justify-center rounded-xl text-muted hover:bg-surface-2 lg:hidden" aria-label="Retour"><FaArrowLeft size={14} /></Button>
        ...
        {conversation.type === 'group' && <Button type="Button" variant="ghost" onClick={onOpenGroupInfo} className="flex h-9 w-9 items-center justify-center rounded-xl text-muted hover:bg-surface-2" aria-label="Options du groupe"><FaEllipsisV size={13} /></Button>}
      </header>
