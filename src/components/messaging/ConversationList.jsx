...
export default function ConversationList({ conversations, contacts, selectedId, search, onSearch, onSelect, onContactSelect, onNew, loading }) {
  ...
  return (
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden border-line bg-surface lg:w-[340px] lg:flex-shrink-0 lg:border-r">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {filteredContacts.map((user) => <Button key={user.uid} type="Button" variant="ghost" onClick={() => onContactSelect(user.uid)} className="flex w-full items-center gap-3 border-b border-line bg-white px-3 py-3 text-left text-ink transition hover:bg-surface-2"><div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{user.name.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ink">{user.name}</p><p className="truncate text-xs text-muted">{user.matricule ? `Matricule : ${user.matricule}` : user.role}</p></div><FaUser size={12} className="text-muted" /></Button>)}
